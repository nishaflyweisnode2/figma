...
customerworkmodel = require("../model/customer_work_model")
...
const laboursignin = (req, res) => {
    ...
    labourmodel.find({ mobilenumber: mobilenumber, password: password, isDeleted: false, usertype: "labour" })
        .then((labour) => {
            if (labour.length > 0) {
...
const acceptworkbylabour = (req, res) => {
    ...
    const labourid = req.session.labourdetails.labourid;
    const d = new Date();
